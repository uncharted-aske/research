...
# %%
# Calculate intermediate data or reload data
i = 'reload'
# %%
...
if i == 'reload':
    ...
    data = []
    with open('./dist/v1/emmaa_5_clusterEdges.pkl', 'rb') as x:
        try:
            while True:
    ...
    ontoClusters_ref, edgesKB_nodes, siblings_ref, siblings_id, siblings_nodeIDs, parent_nodeIDs = data
    data = None
    del data
    ...
    with open('./dist/v1/emmaa_5_clusterEdges.pkl', 'wb') as x:
        for y in [ontoClusters_ref, edgesKB_nodes, siblings_ref, siblings_id, siblings_nodeIDs, parent_nodeIDs]:
            pickle.dump(y, x)
...
Y = [[np.flatnonzero(X[i] & emlib.match_arrays(edgesKB_nodes[:, 1], nodes)) for nodes in siblings_nodeIDs[i][:n]] for i, __ in enumerate(ontoClusters[:1])]
